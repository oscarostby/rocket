...
export function createGamePage(root) {
  ...
  builder.initDefault('mk1-pod', true);
  ...
  Object.entries(PARTS).forEach(([group, parts]) => {
    ...
    parts.forEach((part) => {
      ...
      item.innerHTML = `<div class="part-head"><em class="part-dot" style="background:${part.color || '#93c5fd'}"></em><strong>${part.name}</strong></div><span>${part.mass.toFixed(2)} t · ${part.thrust || 0} kN</span>`;
